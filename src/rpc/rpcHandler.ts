...
export class RpcHandler implements IRpcEndpoint {
  ...
  async eth_estimateUserOperationGas(
    ...
    if (
      ...
      paymasterVerificationGasLimit =
        executionResult.data.executionResult.paymasterVerificationGasLimit ||
        1n;
  ...
  async eth_sendUserOperation(
    ...
    this.logger.warn("send user operation");

    try {
      this.logger.warn("add to mempool");
      ...
      this.logger.warn("got hash " + hash);
      return hash;
    } catch (error) {
      status = "rejected";
      throw error;
  ...
  async addToMempoolIfValid(
    ...
    const entryPointContract = getContract({
      address: entryPoint,
      abi: isVersion06(userOperation) ? EntryPointV06Abi : EntryPointV07Abi,
      client: {
